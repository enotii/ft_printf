...
void	exac_float(t_printf *storage, t_help_struct *lst, int flag, char *rou)
{
	...
	(lst->space > 0 || flag) && storage->precision > 0 ? --storage->precision : 0;
...
void	calc(t_printf *storage, t_help_struct *lst, int flag)
{
	...
	flag || storage->plus || (storage->space && !storage->plus && storage->zero && storage->width > check) ? ++check : 0;
	storage->minus ? storage->zero = 0 : 0;
	!storage->zero && storage->width > check ? lst->space = storage->width - check : 0;
	storage->zero && storage->width > check ? lst->zero = storage->width - check : 0;
}