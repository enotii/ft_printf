...
static void	rounding(t_printf *storage, t_help_struct *lst, double num, int flag)
{
...
static void	ft_transform_float(t_printf *storage, t_help_struct *lst, double num, int flag)
{
	if ((lst->number + 1) % 2 == 0)
...
int print_float(t_printf *storage)
{
	...
	!storage->precision ? ft_transform_float(storage, &lst, num, flag) : rounding(storage, &lst, num, flag);
	return (1);
}