...
/*   Updated: 2019/07/07 18:11:26 by Alexandr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int nan_checker(t_printf *storage, double num)
{
	...
	if (len > 0)
	{
		...
		!storage->minus && !what && storage->space && INF ? write_into_buff(storage, 1, ' ') : 0;
		while (*str)
		{
...
static void	round_long(t_printf *storage, t_help_struct *lst, double num, int flag)
{
...
static void     transform_long_float(t_printf *storage, t_help_struct *lst, long double num, int flag)
{
	if ((lst->number + 1) % 2 == 0)
...
int print_long_float(t_printf *storage)
{
	...
	!storage->precision ? transform_long_float(storage, &lst, num, flag) : round_long(storage, &lst, num, flag);
	return (1);
}