...
/*   Updated: 2019/07/06 01:08:31 by mbeahan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void print_char_minus(t_printf *storage, char symb)
{
    ...
    if (storage->width < 2)
        return ;
    --storage->width;
    storage->zero ? write_into_buff(storage, storage->width, '0') : write_into_buff(storage, storage->width, ' ');
}
...
int print_string(t_printf *storage)
{
    ...
    len > storage->precision && storage->precision_status ? len = storage->precision : 0;
    storage->minus ? write_into_str(start, len, storage) : 0;
    if (len < storage->width)
    {
        storage->width -= len;
        !storage->zero  || storage->minus ? write_into_buff(storage, storage->width, ' ') : write_into_buff(storage, storage->width, '0');
    }
    !storage->minus ? write_into_str(start, len, storage) : 0;
    return (1);
}
...
int print_char(t_printf *storage)
{
    ...
    storage->minus ? print_char_minus(storage, symb) : print_char_plus(storage, symb);
    return (1);
}